...
**Look and Feel:** Users interact with LogDoc through a single chat interface: they ask playbook questions, paste telemetry logs, or ask general SOC questions (e.g. “How do I respond to a brute-force attack?”). Responses stream in real time. The agent uses tools in the background (searching the playbook knowledge base, and optionally the web), and the UI shows brief status hints (e.g. “Searching knowledge base...”) so the analyst sees that answers are grounded in validated sources. Outputs are structured where it matters: classification, MITRE technique, rationale, and recommended actions for triage-style queries; for playbook questions, answers cite or summarize content from the ingested runbooks.
...
![Agent Architecture V1](images/SOC_Agent_Architecture_V1.png)

### Future Agent Architecture
![Agent Architecture V2](images/SOC_Agent_Architecture_V2.png)
...
| **Orchestration** | LangChain / LangGraph (agent loop + tools) | Allows easy definition of single agent with tools and streaming today, and will support a supervisor + specialist graph in the next iteration with minimal rework |
...
**RAG** = embedding model + vector store + retriever (naive, parent-document) + retrieve-then-answer chain
...
- **Retrieval:** For a given query (from the agent’s tool call), the retriever returns the top-k most similar chunks from the vector store. Retrievers that can be used include a naive retrieval or a parent-document retrieval.
- **RAG chain:**  (1) Takes a question, (2) runs retrieval, (3) formats the retrieved chunks as context, (4) calls an LLM with a prompt that says “answer using this context,” and (5) returns the model response (and optionally the retrieved context for evaluation). This chain is invoked by the agent as a tool. The agent does not run RAG by default on every turn as it decides when to call the tool.
...
Playbook content is narrative and section-based (headings, bullet lists, step-by-step procedures). Splitting on `\n\n` and `\n` first keeps intact “one idea per chunk” (e.g. a procedure or a section) and avoids cutting in the middle of a sentence when possible. The 1000-character target keeps each chunk small enough to fit several into the LLM context without blowing the window, while 300 characters of overlap reduces the chance that a key phrase or step is split across a boundary and lost at retrieval. With each playbook ranging from 10000-30000 characters, this initial strategy should be a useful baseline.

### Data Sources

#### Knowledge Base

- Serves as the primary RAG data source, which is generated by ingesting URL content and PDFs. PDFs are converted to markdown and cleaned to remove characters that don't play nicely with JSON. Web pages are loaded with LangChain's `WebBaseLoader`.
- This knowledge base is the authoritative, internal knowledge the agent uses to answer playbook and incident response questions. It is loaded at retriever build time, chunked with the strategy above, embedded, and indexed in the vector store (Qdrant). No additional calls are made at query time once the retriever index is fully built (one-time startup cost).
...
2. Agent interprets the message and may call one or both tools in one or more steps:
...
Parent-Document retrieval is selected as an advanced retriever because it works well for playbooks. The child query will often match to a small part within a playbook but the additional context from the surrounding section is helpful to the overall answer. Additionally, this large context reduces any missing steps within the playbook. For example, the child retriever may extract a section on "Detection", but may not include a section on "Containment." This is where the parent context is helpful. Lastly, this retriever will work better assuming the playbook documents are already split into sections.
...
- Parent-document retrieval improves context recall by returning the parent document around each matched child chunk, so more of the information behind the reference answer is available. Context precision and answer relevancy stay high and faithfulness improves slightly. The tradeoff is higher noise sensitivity. The extra parent context can include irrelevant sections, and the model is more sensitive to that noise. As expected, the token usage is about twice that of naive retrieval given that two retrievers are necessary for parent-document retrieval. Lastly, latency is not impacted, primarily because the two retrievers are called simulataneously.
- For production systems:
    - If cost is a concern, naive retrieval is the best option as the critical metrics are high (context precision, answer relevancy, and faithfulness), whcih helps to gain analyst's trust.
...
## Next Steps
The playbook RAG implementation is a core feature of this project so it is required for the Demo Day project. Playbooks and other authoritative cybersecurity docs (CISA, NIST) will also be included, as they bring additional general-purpose knowledge on how to manage cybersecurity issues. Additionally, the agent orchestration will be updated to better manage the different types of incoming requests. This includes support for supervisor and specialist nodes as well as a response generation node to summarize and synthesize all retrieved data and context.
